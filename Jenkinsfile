    ...
    stage('deploy') {
        withCredentials(bindings: [sshUserPrivateKey(credentialsId: 'insightsbot',
            ...
            sh '''
                eval `ssh-agent`
                ssh-add "$insightsbot"
                rsync -arv -e "ssh -2" * sshacs@unprotected.upload.akamai.com:/114034/insightsbeta/platform/inventory/
                rsync -arv -e "ssh -2" * sshacs@unprotected.upload.akamai.com:/114034/insights/platform/inventory/
            '''
